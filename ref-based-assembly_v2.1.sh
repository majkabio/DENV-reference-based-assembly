...
do
	...
	echo ${line} ${ref_length} ${reads} ${mapped_reads} ${cov_1x} ${perc_cov_1x} ${cov_20x} ${perc_cov_20x} | tr " " "\t" > ref_based_assembly_${name}/alignment_stats/${name}-${line}-stats.temp
done

cat ref_based_assembly_${name}/alignment_stats/*.temp > ref_based_assembly_${name}/alignment_stats/${name}-stats.txt
sed -i '1i Reference\tSize\tReads\tMapped_reads\tCoverage_1X\tPercent_cov_1X\tCoverage_20X\tPercent_cov_20X' ref_based_assembly_${name}/alignment_stats/${name}-stats.txt
